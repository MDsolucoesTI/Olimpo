...
var
  FrmrelboletoTXT: TFrmrelboletoTXT;
...
procedure TFrmrelboletoTXT.BtnimprimirClick(Sender: TObject);
var F : TextFile;
    Linha, brn:string;
    i:integer;
begin
...
procedure TFrmrelboletoTXT.BtnSairClick(Sender: TObject);
begin
 close;
end;

procedure TFrmrelboletoTXT.cmbClassifChange(Sender: TObject);
begin
    If cmbClassif.Text= 'Por Cliente' Then
   Begin
...
procedure TFrmrelboletoTXT.edClienteExit(Sender: TObject);
begin
    if dmdados.TbCliente.Locate('nome',edCliente.Text,[]) then
   edit1.Text:=dmdados.TbClientecodcli.AsString;
end;

procedure TFrmrelboletoTXT.edCcustoChange(Sender: TObject);
 begin
if dmdados.TbCcusto.Locate('Descricao',edCcusto.Text,[]) then
   edit2.Text:=dmdados.TbCcustoCodCcusto.AsString;
end;

function tfrmRelBoletoTXT.AjustaStr( str:String;tam:Integer;carac:char;cont:char):String;
var I:integer;
begin
i:=Length(str);
...
Function tfrmRelBoletoTXT.AjustaDt(str:string):string;
begin
   delete(str,3,1);
   delete(str,5,1);
   result:=str;
...
Function tfrmRelBoletoTXT.RetiraVirg(str:string):string;
begin
...
procedure TFrmrelboletoTXT.FormClose(Sender: TObject;
...
procedure TFrmrelboletoTXT.EdtInicialExit(Sender: TObject);
begin
...
procedure TFrmrelboletoTXT.EdtFinalExit(Sender: TObject);
var flag:boolean;
begin
  flag:=true;
  if (EdtFinal.Text ='  /  /    ') then
...
procedure TFrmrelboletoTXT.EdtFinalChange(Sender: TObject);
begin
 Btnimprimir.Enabled:=true;
end;

procedure TFrmrelboletoTXT.FormShow(Sender: TObject);
begin
FrmPrincipal.StatusTeclas(True,'[F2] Gravar [Esc] Cancelar ou Sair');
    //CmbDestino.Enabled:=false;
end;

procedure TFrmrelboletoTXT.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
